...
# data<-reviews
# data <- read.csv("SReview.csv", na = "-", fileEncoding = "CP949", encoding = "UTF-8")
...
t2 <- Sys.time()
t2 - t1  # about 4 minutes on laptop

theta <- t(apply(fit$document_sums + alpha, 2, function(x) x/sum(x)))
phi <- t(apply(t(fit$topics) + eta, 2, function(x) x/sum(x)))

SReview <- list(phi = phi,
                     theta = theta,
...
json <- createJSON(phi = SReview$phi, 
                   theta = SReview$theta, 
                   doc.length = SReview$doc.length, 
                   vocab = SReview$vocab, 
                   term.frequency = SReview$term.frequency, encoding='UTF-8')
